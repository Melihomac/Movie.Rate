...
const useHookNewMovie = () => {
  const [dataComing, setData] = useState([]);
  const [isLoadingComing, setIsLoading] = useState(false);
  const [errorComing, setError] = useState(null);
  const options = {
    method: 'GET',
  ...
  return {dataComing, isLoadingComing, errorComing, refetch};
};
