...
const useHookNewMovie = () => {
  const [dataNewMovie, setDataNewMovie] = useState([]);
  const [isLoadingNewMovie, setIsLoading] = useState(false);
  const [errorNewMovie, setError] = useState(null);
  const options = {
    method: 'GET',
  ...
  return {dataNewMovie, isLoadingNewMovie, errorNewMovie, refetch};
};
